...
function debug() {
    ...
    dotnet build "$BASE_DIRECTORY/src/Medo.Twofish.sln" \
                 --framework "net7.0" \
                 --configuration "Debug" \
                 --output "$BASE_DIRECTORY/build/debug/" \
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/debug/Medo.Twofish.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/debug/Medo.Twofish.pdb" "$BASE_DIRECTORY/bin/" || return 1
...
function release() {
    ...
    dotnet build "$BASE_DIRECTORY/src/Medo.Twofish.sln" \
                 ...
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/release/Medo.Twofish.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/release/Medo.Twofish.pdb" "$BASE_DIRECTORY/bin/" || return 1
    echo "${ANSI_CYAN}Output in 'bin/'${ANSI_RESET}"
}
...
function package() {
    ...
    dotnet pack "$BASE_DIRECTORY/src/Medo.Twofish.csproj" \
                --configuration "Release" \
                --force \
                --include-source \
...
function test() {
    ...
    echo ".NET `dotnet --version`"
    dotnet test "$BASE_DIRECTORY/src/Medo.Twofish.sln" \
                --configuration "Debug" \
...
PACKAGE_ID=`cat "$BASE_DIRECTORY/src/Medo.Twofish.csproj" | grep "<PackageId>" | sed 's^</\?PackageId>^^g' | xargs`
PACKAGE_VERSION=`cat "$BASE_DIRECTORY/src/Medo.Twofish.csproj" | grep "<Version>" | sed 's^</\?Version>^^g' | xargs`
